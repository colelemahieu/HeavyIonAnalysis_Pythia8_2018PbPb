...
process.source = cms.Source("PoolSource",
    ...
    fileNames = cms.untracked.vstring('file:/eos/cms/store/group/phys_heavyions/clemahie/RECO/pythiaDigi/MinBias_PbPb_5p36TeV_Hydjet_RECO_Run2/240613_125320/0000/stepAODMINIAOD.root')
)
...
process.ak04PFJets = cms.EDProducer("CSJetProducer",
...
process.ak04PFpatJetFlavourAssociationLegacy = cms.EDProducer("JetFlavourIdentifier",
    physicsDefinition = cms.bool(False),
    srcByReference = cms.InputTag("ak04PFpatJetPartonAssociationLegacy")
)


process.ak04PFpatJetGenJetMatch = cms.EDProducer("GenJetMatcher",
...
process.ak04PFpatJetPartonAssociationLegacy = cms.EDProducer("JetPartonMatcher",
...
process.ak04PFpatJetPartonMatch = cms.EDProducer("MCMatcher",
    checkCharge = cms.bool(False),
...
process.ak04PFpatJetPartons = cms.EDProducer("HadronAndPartonSelector",
...
process.jetsR4 = cms.Sequence(process.ak04PFJets+process.ak04PFpatJetCorrFactors+process.ak04PFpatJetPartonMatch+process.ak4GenJetsNoNu+process.ak04PFpatJetGenJetMatch+process.ak04PFpatJetPartonAssociationLegacy+process.ak04PFpatJetFlavourAssociationLegacy+process.ak04PFpatJetPartons+process.ak04PFpfImpactParameterTagInfos+process.ak04PFpfSecondaryVertexTagInfos+process.ak04PFpfDeepCSVTagInfos+process.ak04PFpfDeepCSVJetTags+process.ak04PFpfJetProbabilityBJetTags+process.ak04PFpatJets)
...
process.forest = cms.Path(process.HiForestInfo+process.l1object+process.trackSequencePbPb+process.particleFlowAnalyser+process.hiEvtAnalyzer+process.HiGenParticleAna+process.unpackedMuons+process.correctedElectrons+process.ggHiNtuplizer+process.muonAnalyzer+process.extraJetsMC+process.jetsR4+process.ak4PFJetAnalyzer)
