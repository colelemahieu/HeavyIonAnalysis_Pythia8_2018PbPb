...
void miniAOD_mcSkim_jets(string inputfile, string outfile)
{    
  ...
  // histogram
  TH1F *h_gen_from_reco_30_32 = new TH1F("h_gen_from_reco_30_32","h_gen_from_reco_30_32",80,10,40);
  TH1F *h_gen_from_reco_20_22 = new TH1F("h_gen_from_reco_20_22","h_gen_from_reco_20_22",80,10,30);
  ...
  for (int i=halfEntries; i<jetentries; i++)
    {
      ...
      // investigating which gen jts make 30 GeV reco jets
      if (jetPt[0]>30 && jetPt[0]<32) h_gen_from_reco_30_32->Fill(genPt[0]);
      if (jetPt[1]>20 && jetPt[1]<22) h_gen_from_reco_20_22->Fill(genPt[1]);
  ...
  h_gen_from_reco_30_32->Write();
  h_gen_from_reco_20_22->Write(); 
  newfile.Write();
  newfile.Close();
}
