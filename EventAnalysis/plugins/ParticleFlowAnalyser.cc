...
void ParticleFlowAnalyser::analyze(const edm::Event& iEvent, const edm::EventSetup&) {
  ...
  for (const auto& pfcand : *pfCandidates) {
    ...
    if (pfcand.bestTrack())
    {
      pfTrkPt_.push_back(pfcand.bestTrack()->pt());
      pfTrkEta_.push_back(pfcand.bestTrack()->eta());
    }
    else
    {
      pfTrkPt_.push_back(-99);
      pfTrkEta_.push_back(-99);
    }
...
void ParticleFlowAnalyser::beginJob() {
  ...
  tree_->Branch("pfEt", &pfEt_);
  tree_->Branch("pfE", &pfE_);
  tree_->Branch("pfM", &pfM_);
  tree_->Branch("pfTrkPt", &pfTrkPt_);
  tree_->Branch("pfTrkEta", &pfTrkEta_);
}
