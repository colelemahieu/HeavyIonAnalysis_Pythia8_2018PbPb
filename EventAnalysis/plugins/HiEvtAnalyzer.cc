...
HiEvtAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(doMC_){
    std::cout << "This is the doMC loop" << std::endl;
    if(useHepMC_) {
      std::cout << "This is the if useHepMC loop" << std::endl;
      ...
      std::cout << "This is the useHepMC else loop" << std::endl;
      edm::Handle<GenEventInfoProduct> genInfo;
      if(iEvent.getByToken(genInfoToken_, genInfo)) {
	std::cout << "This is the if(iEvent.getByToken(genInfoToken_, genInfo)) loop" << std::endl;
        proc_id = genInfo->signalProcessID();
        if (genInfo->hasBinningValues()){
	  pthat = genInfo->binningValues()[0];
	  std::cout << "made it to the genInfo->hasBinningValues() if statement" << std::endl;
	}
